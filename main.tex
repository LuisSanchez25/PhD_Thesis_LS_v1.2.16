...
\newacronym[description={When an S1 is matched with an S2 of a different origin to form an event due to misconstruction of the interaction}]{ac}{AC}{Accidental Coincidences}

%\newacronym{bipo}{BiPo}{$^{214}$Bi $^{214}$Po}

\newacronym[description={The neuron in a SOM whose weight vector is closest to the input vector, determining where the input maps on the grid}]{bmu}{BMU}{Best Matching Unit}

\newacronym[description={A unified energy scale that combines the S1 and S2 signals to provide improved energy resolution for detector events}]{ces}{CES}{Combined Energy Spectrum}

\newacronym[description={A weak interaction process where a neutrino scatters off an entire nucleus coherently, producing a nuclear recoil signal similar to WIMP interactions}]{ceνns}{CEνNS}{Coherent Elastic Neutrino-Nucleus Scattering}

\newacronym[description={The thermal radiation left over from the early universe, providing evidence for the Big Bang and dark matter}]{cmb}{CMB}{Cosmic Microwave Background}

\newacronym[description={A symmetry operation combining charge conjugation and parity transformation in particle physics}]{cp}{CP}{Charge-Parity}

\newacronym[description={S1 signal after the corresponding corrections have been applied}]{cs1}{cS1}{Corrected Scintillation Signal}

\newacronym[description={S2 signal after the corresponding corrections have been applied}]{cs2}{cS2}{Corrected Ionization Signal}

\newacronym[description={The hardware and software system responsible for digitizing and recording detector signals from PMTs and other sensors}]{daq}{DAQ}{Data Acquisition System}

\newacronym[description={Non-luminous matter hypothesized to account for gravitational effects observed in galaxies and cosmological structures that cannot be explained by visible matter alone}]{dm}{DM}{Dark Matter}

\newacronym[description={Energy deposition from particles that primarily interact with atomic electrons, such as gamma rays and beta particles, producing a characteristic signal ratio in dual-phase xenon detectors}]{er}{ER}{Electronic Recoil}


\newacronym[description={A noble gas used as the detection medium in xenon-based dark matter experiments}]{gxe}{GXe}{Gaseous Xenon}

\newacronym[description={The fraction of scintillation photons successfully detected by the photomultiplier tubes}]{lce}{LCE}{Light-Collection Efficiency}

\newacronym{lngs}{LNGS}{Laboratori Nazionali del Gran Sasso (Gran Sasso National Laboratory)}

\newacronym[description={The liquid phase of xenon used as the primary detection medium in dual-phase xenon detectors}]{lxe}{LXe}{Liquid Xenon}

\newacronym[description={A subset of artificial intelligence that enables systems to learn and improve from experience without being explicitly programmed}]{ml}{ML}{Machine Learning}

\newacronym{nb}{NB}{Natural Breaks}

\newacronym[description={A computational model inspired by biological neural networks that can learn complex patterns in data}]{nn}{NN}{Neural Network}

\newacronym[description={Energy deposition from particles that scatter off atomic nuclei, such as neutrons or WIMPs, producing a distinct signal ratio compared to electronic recoils}]{nr}{NR}{Nuclear Recoil}

\newacronym[description={A distributed computing infrastructure providing computational resources for scientific research}]{osg}{OSG}{Open Science Grid}

\newacronym[description={An electron emitted from a material after absorbing a photon of light}]{pe}{PE}{Photoelectron}

\newacronym[description={A highly sensitive light detector that amplifies the signal from individual photons}]{pmt}{PMT}{Photo-Multiplier Tube}

\newacronym[description={A synthetic fluoropolymer used as a reflective coating in xenon detectors}]{ptfe}{PTFE}{Polytetrafluoroethylene (Teflon)}

\newacronym[description={A technique that distinguishes particle types based on the temporal profile of scintillation signals, exploiting differences in light emission timing}]{psd}{PSD}{Pulse Shape Discrimination}

\newacronym[description={A particle detector that uses electric fields to drift ionization electrons to a detection plane, enabling 3D position reconstruction from drift time and hit pattern}]{tpc}{TPC}{Time-Projection Chamber}

\newacronym{rcc}{RCC}{Research Computing Center}

\newacronym{rse}{RSE}{Rucio Storage Element}

\newacronym[description={The light signal produced by ionized xenon atoms de-exciting to their ground state}]{s1}{S1}{Scintillation Signal}

\newacronym[description={The electric signal produced by drifting ionization electrons in the xenon detector}]{s2}{S2}{Ionization Signal}

\newacronym[description={The response from a single photoelectron, used as a fundamental unit for detector calibration}]{se}{SE}{Single Electron}

\newacronym[description={An unsupervised machine learning algorithm that creates a low-dimensional representation of high-dimensional input data}]{som}{SOM}{Self-Organizing Map}

\newacronym[description={A variant of the Self-Organizing Map that includes a conscience mechanism to improve training efficiency and convergence}]{csom}{CSOM}{Conscience Self-Organizing Map}

\newacronym[description={A designated period of detector operation with stable conditions used for physics data collection and analysis}]{sr}{SR}{Science Run}

\newacronym[description={A supervised machine learning algorithm that constructs optimal hyperplanes in high-dimensional feature space for classification and regression tasks}]{svm}{SVM}{Support Vector Machine}

\newacronym[description={A data compression technique that represents high-dimensional vectors by mapping them to a finite set of prototype vectors or codebook entries}]{vq}{VQ}{Vector Quantization}

\newacronym[description={A hypothetical particle candidate for dark matter that interacts via the weak nuclear force and gravity, with mass typically in the GeV to TeV range}]{wimp}{WIMP}{Weakly-Interactive Massive Particle}

% Duplicate glossary entries removed - descriptions now consolidated into acronym definitions above
...
\begin{document}
    ...
    \huge\bfseries
     Advancing Dark Matter Detection: Self-Organizing Maps for Signal Classification in Xenon TPCs \\
    ...
    \noindent\rule{8cm}{0.4pt}\\
    Dr. Aaron Higuera Pichardo\\
    Assistant Research Professor of Physics and Astronomy
    \end{flushleft}
...
\begin{abstract}
Dark matter constitutes approximately 27\% of the universe's mass-energy content, yet its particle nature remains unknown. Weakly Interacting Massive Particles (WIMPs) represent a theoretically well-motivated candidate, and dual-phase liquid xenon time projection chambers (TPCs) such as XENONnT are among the most sensitive instruments for their direct detection. Accurate classification of scintillation (S1) and ionization (S2) signals is fundamental to these searches, as misclassification increases background rates and reduces sensitivity to rare WIMP-nucleon interactions.           
   
This thesis develops and validates a Self-Organizing Map (SOM)-based approach for signal classification in xenon TPC experiments. Using Conscience SOMs (CSOMs), an unsupervised neural network that preserves topological relationships in high-dimensional data, we trained classifiers on XENONnT calibration and background data. The SOM-aided classification reduced misclassified single electrons by 50\% and misclassified S1 signals by approximately 30\% compared to the previous algorithm, directly decreasing the accidental coincidence background critical to low-energy physics searches including WIMP detection and solar neutrino measurements.                                    
Beyond classification improvement, the SOM analysis identified 22 distinct signal clusters, revealing detector artifacts, multi-scatter events, and gas-phase interactions that inform data quality cuts and detector characterization. We validated the generalizability of this approach on XAMS, a    smaller R&D detector at Nikhef, demonstrating that SOM-based methods successfully identify signal types across different xenon TPC implementations. Additionally, we developed sciSOM, an open-source Python package for CSOM training and visualization, and contributed to xedocs, a metadata management system ensuring data integrity across XENONnT science runs.                                         
This work establishes unsupervised neural networks as robust tools for signal classification in rare-event searches, with direct applicability to next-generation experiments such as XLZD. 
\end{abstract}
